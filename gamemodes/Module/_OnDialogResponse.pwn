public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == 8000) //am()
	{
		if(response)
		{
			if(listitem == 0)
			{
				ProxDetector(60.0, playerid, "[POLICE] YEU CAU PHUONG TIEN PHIA TRUOC TAP XE VAO LE", COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW,1);
			}
			if(listitem == 1)
			{
				ProxDetector(60.0, playerid, "[POLICE] YEU CAU NGUOI DAN KHONG LIEN QUAN ROI KHOI KHU VUC NAY", COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW,1);
			}
			if(listitem == 2)
			{
				ProxDetector(60.0, playerid, "[POLICE] YEU CAU BO VU KHI XUONG VA DUA HAI TAY LEN DAU", COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW, COLOR_YELLOW,1);
			}
		}
	}
		else if(dialogid == DIALOG_TIMDUONG)
	{
		if(response)
		{
			if(listitem == 0)
			{
			    SetPlayerCheckpoint(playerid, 1811.1061, -1576.4225, 13.5203, 5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 1)
			{
			    SetPlayerCheckpoint(playerid, 2027.0698, -1421.1975, 16.9922, 5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 2)
			{
			    SetPlayerCheckpoint(playerid, 1542.7155,-1675.0443,13.5550,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 3)
			{
			    SetPlayerCheckpoint(playerid, 2073.8018,-1831.9414,13.5469,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 4)
			{
			    SetPlayerCheckpoint(playerid, 2222.2019,-1727.4065,13.3967,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 5)
			{
			    SetPlayerCheckpoint(playerid, 1831.8413,-1842.9044,13.5781,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 6)
			{
			    SetPlayerCheckpoint(playerid, 1756.8284,-1696.6349,13.5241,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 7)
			{
			    SetPlayerCheckpoint(playerid, 1503.3619,-1589.9006,13.3828,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 8)
			{
			    SetPlayerCheckpoint(playerid, 1850.0396,-1583.1266,13.6417,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
			if(listitem == 9)
			{
			    SetPlayerCheckpoint(playerid, 1450.0885,-1592.9258,13.3828,5.0);
				SetPVarInt(playerid, "timduong", 1);
			}
		}
	}

	if(dialogid == DIALOG_SHOPMOBI)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0: // Car Voucher
				{
					if(PlayerInfo[playerid][pKimCuong] >= 450)
					{
						...
						Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua phieu ~g~CAR ~w~voi gia ~r~450 kim cuong");
					}
	...
	if(dialogid == DIALOG_NHAPMATKHAU)
	{
		SetPVarInt(playerid, "DaNhapMK" , 1);
		new str[50], str1[13];
	    format(str, sizeof(str) ,"%s", inputtext);
	    SetPVarString(playerid,"Matkhau",str);
	    format(str1, sizeof(str1), "xxxxxxxxxxxx");
	    if(IsPlayerMobile(playerid)) //Mobi
		{
			PlayerTextDrawSetString(playerid, LoginMobi[playerid][8], str1);
	    	PlayerTextDrawShow(playerid, LoginMobi[playerid][8]);
		} else { //PC
			PlayerTextDrawSetString(playerid, loginpanel_PTD[playerid][2], str1);
	    	PlayerTextDrawShow(playerid, loginpanel_PTD[playerid][2]);
		}
	}
	if(dialogid == DIALOG_NHAPMATKHAU1)
	{
		SetPVarInt(playerid, "DaNhapMK" , 1);
		new str[50], str1[13];
	    format(str, sizeof(str) ,"%s", inputtext);
	    ...
	    if(IsPlayerMobile(playerid)) //Mobi
		{
			PlayerTextDrawSetString(playerid, RegisterMobi[playerid][10], str1);
	    	PlayerTextDrawShow(playerid, RegisterMobi[playerid][10]);
		} else { //PC
			PlayerTextDrawSetString(playerid, RegisterPC[playerid][2], str1);
	    	PlayerTextDrawShow(playerid, RegisterPC[playerid][2]);
		}
	}
	if(dialogid == DIALOG_NHAPEMAIL)
	{
		SetPVarInt(playerid, "DaNhapEmail" , 1);
		new str[50], str1[50];
	    format(str, sizeof(str) ,"%s", inputtext);
	    SetPVarString(playerid,"NhapEmail",str);
	    format(str1, sizeof(str1), "%s", inputtext);
	    if(IsPlayerMobile(playerid)) //Mobi
		{
			PlayerTextDrawSetString(playerid, RegisterMobi[playerid][11], str1);
	    	PlayerTextDrawShow(playerid, RegisterMobi[playerid][11]);
		} else { //PC
			PlayerTextDrawSetString(playerid, RegisterPC[playerid][7], str1);
	    	PlayerTextDrawShow(playerid, RegisterPC[playerid][7]);
		}
	}
            ...
            switch(listitem)
            {
                case 0: // 9mm
				{
		            if(PlayerInfo[playerid][pMats] >= 1000)
		            {
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da thay dan 9mm.");
					}
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de mua dan.");
				}
				case 1:
                {
				...
				case 2:
                {
                    ...
                    {
                        PlayerInfo[playerid][pMats] -= 1000;
                        GivePlayerAmmoEx(playerid, 25, 7);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da thay dan shotgun.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 3:
				{
			       	if(PlayerInfo[playerid][pMats] >= 1000)
			        {
                		...
                		GivePlayerAmmoEx(playerid, 28, 30);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da thay dan Uzi.");
				    }
                	else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 4:
				{
					...
					{
                        PlayerInfo[playerid][pMats] -= 1000;
                        GivePlayerAmmoEx(playerid, 32, 7);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan TEC-9.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 5:
				{
					if(PlayerInfo[playerid][pMats] >= 1000)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan MP5.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 6:
				{
					if(PlayerInfo[playerid][pMats] >= 1000)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan Deagle");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 7:
				{
					...
					{
                    	PlayerInfo[playerid][pMats] -= 1000;
                    	GivePlayerAmmoEx(playerid, 33, 7);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan Rilfe.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 8:
				{
					if(PlayerInfo[playerid][pMats] >= 1000)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan AK-47.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 9:
				{
					if(PlayerInfo[playerid][pMats] >= 1000)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan M4.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 10:
				{
					...
					{
                        PlayerInfo[playerid][pMats] -= 1000;
                        GivePlayerAmmoEx(playerid, 27, 30);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan Combat Shotgun.");
				    }
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
				case 11:
				{
					...
					{
                    	PlayerInfo[playerid][pMats] -= 1000;
                    	GivePlayerAmmoEx(playerid, 34, 30);
						SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da che tao bang dan Sniper Rilfe.");
					}
					else SendClientMessageEx(playerid, COLOR_YELLOW, "{F00A0A}[?]{EAFF00}Ban khong du vat lieu de che tao dan.");
				}
            } // end switch case
	...
	if(dialogid == DIALOG_BANGXEPHANG)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0: // Tai san - Money
				{
					DisplayBangXepHangTaiSan(playerid);
				}
				case 1: // Cap do - Level
				{
					DisplayBangXepHangCapDo(playerid);
				}
				case 2: // Kim Cuong 
				{
					DisplayBangXepHangKimCuong(playerid);
				}
			}
		}
		return 1;
	...
	if(dialogid == DIALOG_FINDJOB)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0://Pizza
				{
					SetPlayerCheckpoint(playerid, POS_PIZZA, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 1://Trucker
				{
					SetPlayerCheckpoint(playerid, POS_TRUCKER, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 2://Fishermen
				{
					SetPlayerCheckpoint(playerid, POS_FISHERMEN, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 3://Worker
				{
					SetPlayerCheckpoint(playerid, POS_WORKER, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 4://chat go
				{
					SetPlayerCheckpoint(playerid, -1093.3726,-1288.9056,129.4963, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 5://chat go
				{
					SetPlayerCheckpoint(playerid, 2794.7161,-2396.2034,13.6320, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
				case 6://chat go
				{
					SetPlayerCheckpoint(playerid, 2176.1968,-2258.1108,14.7734, 5.0);
					SetPVarInt(playerid, "FindJob", 1);
					SendClientMessage(playerid, -1, "Dia diem da duoc danh dau tren ban do.");
				}
			}
	...
	if(dialogid == DIALOG_GETJOB)
	{
		if(response)
		{
            if(GettingJob[playerid] > 0) 
			{
				...
				format(string, sizeof(string), "{FFFFFF}Ban da nhan duoc Job {FFFF00}%s\n{FFFFFF}Chuc ban choi game vui ve.", GetJobName(GettingJob[playerid]));
				ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Job", string, "Ok", "");					
			...
			{
				if(GettingJob2[playerid] == 1)
				{
					SetPlayerSkin(playerid, 155);
				}				
				format(string, sizeof(string), "{FFFFFF}Ban da nhan duoc Job {FFFF00}%s\n{FFFFFF}Chuc ban choi game vui ve.", GetJobName(GettingJob2[playerid]));
				ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Job", string, "Ok", "");					
                ...
                SendClientMessageEx(playerid, COLOR_GREY, "Ban khong nhan job nao ca.");
                return 1;
            }
        }
		else
		{
			SendClientMessageEx(playerid, -1, "Tam biet :D");
		}
	}
	...
	if(RegistrationStep[playerid] != 0)
	{
		...
		if(dialogid == REGISTERREF)
		{
			...
			else 
			{
				...
				if(PlayerInfo[playerid][pSex] == 0)//Female
				{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "[-] BAN DA THAM GIA MAY CHU {ff0092}GVRP.VN");
				SendClientMessageEx(playerid, COLOR_WHITE, "[-] {1f9dc4}CHUNG TOI TANG BAN {f6be00} VIP GOLD {1f9dc4} HAY CAY CUOC THAT CHAM CHI NHE.");
				SendClientMessage(playerid, COLOR_WHITE, "[-] Hay {ff5b4f}/myvouchers {FFFFFF} de su dung phieu {f6be00} vip gold.");
				SetCameraBehindPlayer(playerid);
				MessRegister(playerid);
				SetPlayerPos(playerid, 2104.4060,-1762.8926,13.5625);//Dangkytaikhoan
				SetPlayerVirtualWorld(playerid, 0);
	...
	if(dialogid == NULLEMAIL)
	{
		if(response)
		{
			...
			mysql_function_query(MainPipeline, query, false, "OnQueryFinish", "i", SENDDATA_THREAD);
			 ...
			 SendClientMessageEx(playerid, COLOR_YELLOW,"Ban da bi kick vi huy xac minh E-Mail.");
			 Kick(playerid);
		}
	}
	...
	else if(dialogid == DIALOG_CHANGEPASS2)
	{
	    if(response)
	    {
			...
			else if(strcmp(PlayerInfo[playerid][pBirthDate], "1999-12-12", true) == 0 && PlayerInfo[playerid][pTut] != 0) ShowLoginDialogs(playerid, 1);
 	...
 	if(dialogid == FAQMENU)
  	{
   		if(response)
     	{
            ...
            {
           		ShowPlayerDialog(playerid, WEBSITEFAQ, DIALOG_STYLE_MSGBOX, "Thong tin khac", "Thong tin:\n\nForum ho tro: https://discord.gg/jbemhckHa6, tim hieu sau hon: https://discord.gg/jbemhckHa6, Up anh nhanh: https://discord.gg/jbemhckHa6", "Cam on", "Huy bo");
            }
...
/*	if(dialogid == MDC_CHECK && response)
	...
	if(dialogid == DIALOG_ARRESTREPORT)
	{
		if(response)
		{
			...
			else if(PlayerInfo[playerid][pTVCuopNG] == 1)
			{
				PlayerInfo[playerid][pTVCuopNG] = 0;
		        g_mysql_SaveAccount(playerid);
				HideTVParty(playerid);
	...
	else if(dialogid == DMRCONFIRM)
	{
	    if(response)
	    {
			...
			format(string, sizeof(string), "%s(%i) Hacking (thoi gian: %s)", GetPlayerNameEx(giveplayerid), giveplayerid, date(gettime() - ShotPlayer[giveplayerid][playerid], 1));
			SendReportToQue(playerid, string, 2, 1);

			ShotPlayer[giveplayerid][playerid] = 0;
			baocaotime[playerid] = 120;
	    }
	    else
		{
	        ...
	        switch(listitem)
			{
				case 0: //Vi pham luat
			    {
			        if(PlayerInfo[playerid][pJailTime] > 0) {
			        	...
			        	ShowPlayerDialog(playerid, DIALOG_REPORTDM, DIALOG_STYLE_INPUT, "BAO CAO - HACKING", "Nhap ID nguoi choi.", "Dong y", "Huy bo");
					}
				}
			    case 1: //Bug
			    {
			        ...
			        else
			        {
						...
						SetPVarInt(playerid, "_rRepID", playerid);
      					format(Message, sizeof(Message), "{AA3333}AdmWarning{FFFF00}: %s (ID %d) bi freeze. (Falling Report)", GetPlayerNameEx(playerid), playerid);
	...
	else if(dialogid == DIALOG_REPORTDM)
	{
	    if(response)
	    {
            ...
            if(sscanf(inputtext, "u", Player)) {
        		ShowPlayerDialog(playerid, DIALOG_REPORTDM, DIALOG_STYLE_INPUT, "Bao cao nguoi choi - Hacking", "(Error - Invalid Player) Nhap ID nguoi choi.", "Dong y", "Huy bo");
				return 1;
			}
			...
			if(PlayerInfo[playerid][pLevel] < 1) return SendClientMessage(playerid, COLOR_GRAD2, "Ban phai dat level 1 moi su dung duoc lenh nay.");
			...
			ShowPlayerDialog(playerid, DMRCONFIRM, DIALOG_STYLE_MSGBOX, "Hacking Reports", "Neu ban da chac chan nguoi nay hacking , vui long nhan dong y de BQT chung toi lam viec!.", "Dong y", "Huy bo");
	    }
	}
	...
	else if(dialogid == DIALOG_LOADTRUCKOLD) // TRUCKER JOB LOAD TRUCK
	{
 		if(response)
		{
			...
			if(listitem == 1) // Illegal goods
			{
				...
				{
		    		ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi  		{FFFFFF}(Level 2 Bonus: Free Spidtol)\n{FF0606}Drugs 			{FFFFFF}(Level 2 Bonus: Free 4 pot, 2 crack)\n{FF0606}Illegal materials  	{FFFFFF}(Level 2 Bonus: Free 50 materials)", "Lua chon", "Thoat");
				}
