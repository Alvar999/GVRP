...
forward HungerDeathTimer(playerid);
public HungerDeathTimer(playerid) 
{
	if(GetPVarInt(playerid, "IsFrozen") == 1)
	{
...
Player_Notice(playerid, type, string[], time = 5000)
{
	switch(type)
	{
		case AUTO_MESSAGE:
		{
			if(PlayerInfo[playerid][pPlayer_Notice])
			{
				PlayerTextDrawHide(playerid, PlayerInfo[playerid][pTextdraws][2]);
				KillTimer(PlayerInfo[playerid][pPlayer_Notice_Timer]);
			}
			...
			PlayerInfo[playerid][pPlayer_Notice] = true;
			PlayerInfo[playerid][pPlayer_Notice_Timer] = SetTimerEx("HidePlayerNotice", time, false, "d", playerid);
		}
		case AUTO_MESSAGE_BOX:
		{
			if(PlayerInfo[playerid][pNotice_Box])
			{
				PlayerTextDrawHide(playerid, PlayerInfo[playerid][pTextdraws][3]);
				KillTimer(PlayerInfo[playerid][pNotice_Box_Timer]);
			}
			...
			PlayerInfo[playerid][pNotice_Box] = true;
			PlayerInfo[playerid][pNotice_Box_Timer] = SetTimerEx("HidePlayerNoticeBox", time, false, "d", playerid);
		}
	}
	return 1;
}
...
public HidePlayerNotice(playerid)
{
	...
	PlayerInfo[playerid][pPlayer_Notice] = false;
	return PlayerTextDrawHide(playerid, PlayerInfo[playerid][pTextdraws][2]);
}

forward HidePlayerNoticeBox(playerid);
public HidePlayerNoticeBox(playerid)
{
	if(!PlayerInfo[playerid][pNotice_Box])
		return 0;

	PlayerInfo[playerid][pNotice_Box] = false;
	return PlayerTextDrawHide(playerid, PlayerInfo[playerid][pTextdraws][3]);
}

forward CapCuuTimer(playerid, giveplayerid);
public CapCuuTimer(playerid, giveplayerid)
{
	KillEMSQueue(giveplayerid);
	ApplyAnimation(giveplayerid,"COP_AMBIENT","Coplook_watch", 4.0, 0, 0, 0, 0, 0, 1);	
	SetPlayerHealth(giveplayerid, 50);
	SendClientMessage(giveplayerid, -1, "Ban da duoc cap cuu thanh cong va mat 20000 tien phi");
	GivePlayerCash(giveplayerid, -20000);
	new string[1280], ip[32], ipgive[32];
	GetPlayerIp(giveplayerid, ipgive, sizeof(ipgive));
	GetPlayerIp(playerid, ip, sizeof(ip));
	format(string, sizeof(string),"Player: %s (ID: %s) (IP: %s) da cap cuu thanh cong %s (ID: %d) (IP: %s).", GetPlayerNameEx(playerid), playerid, ip, GetPlayerNameEx(giveplayerid), giveplayerid, ipgive);
	Log("logs/Capcuu.log", string);
	if(IsAMedic(playerid))
	{
		SendClientMessageEx(playerid, -1, "{41ac7d}[CAP CUU]{FFFFFF} Ban da nhan duoc 20000");
...
stock GetJobName(job)
{
	...
	switch(job)
	{
		...
		case 24: name = "Chat go";
		case 30: name = "Cong truong";
		case 31: name = "Ngu dan";
		case 32: name = "Boc vac";
		case 33: name = "Container Delivery";
...
stock GetJobLevel(playerid, job)
{
	...
	switch(job)
	{
		case 1:
		{
			new skilllevel = PlayerInfo[playerid][pPizzaSkill];
			if(skilllevel >= 0 && skilllevel <= 100) jlevel = 1;
			if(skilllevel >= 101 && skilllevel <= 200) jlevel = 2;
			if(skilllevel >= 201 && skilllevel <= 500) jlevel = 3;
			if(skilllevel >= 501 && skilllevel <= 1000) jlevel = 4;
			else if(skilllevel >= 1001) jlevel = 5;
		}
		case 2:
		{
		...
		case 3:
		{
			new skilllevel = PlayerInfo[playerid][pFishermenSkill];
			if(skilllevel >= 0 && skilllevel <= 100) jlevel = 1;
			if(skilllevel >= 101 && skilllevel <= 200) jlevel = 2;
			if(skilllevel >= 201 && skilllevel <= 500) jlevel = 3;
			if(skilllevel >= 501 && skilllevel <= 1000) jlevel = 4;
		...
		case 4:
		{
			new skilllevel = PlayerInfo[playerid][pBuilderSkill];
			if(skilllevel >= 0 && skilllevel <= 100) jlevel = 1;
			if(skilllevel >= 101 && skilllevel <= 200) jlevel = 2;
			if(skilllevel >= 201 && skilllevel <= 500) jlevel = 3;
			if(skilllevel >= 501 && skilllevel <= 1000) jlevel = 4;
...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: ", "Quay Lai", "");
		}
...
public InitiateGamemode()
{
	...
	print("Gamemode edited by Dawkin");
	return 1;
}		
...
public Maintenance()
{
	...
	SendRconCommand("password mandeptrai");
	SendRconCommand("hostname [GVRP] Bao tri may chu");
	foreach(new i: Player)
	{
		if(gPlayerLogged{i}) {
	...
	if(GetPVarInt(playerid, "HelpTime") > 0)
 	{
    	...
    	if(GetPVarInt(playerid, "HelpTime") == 0)
     	{
      		SendClientMessageEx(playerid, COLOR_GREY, "Yeu cau tro giup cua ban da het han. Neu day la mot loi hay tham gia discord https://discord.gg/jbemhckHa6 de bao loi.");
...
public LoadTruckOld(playerid)
{
    SetPVarInt(playerid, "LoadTruckTime", GetPVarInt(playerid, "LoadTruckTime")-1);
	new string[128];
	format(string, sizeof(string), "~w~Ban dang lay hang vui long cho ~r~%d~w~ giay con lai", GetPVarInt(playerid, "LoadTruckTime"));
...
{
    ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Houses...", 1);
	SaveHouses();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Dynamic Doors...", 1);
	SaveDynamicDoors();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Map Icons...", 1);
	SaveDynamicMapIcons();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Gates...", 1);
	SaveGates();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Event Points...", 1);
	SaveEventPoints();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Paintball Arenas...", 1);
	SavePaintballArenas();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Server Configuration", 1);
    Misc_Save();
    ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Office Elevator...", 1);
	SaveElevatorStuff();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Mail Boxes...", 1);
	SaveMailboxes();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Plants...", 1);
	SavePlants();
	ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving Speed Cameras...", 1);
	SaveSpeedCameras();
	if(rflstatus > 0) {
		ABroadCast(COLOR_YELLOW, "BAO TRI: Force Saving RFL Teams...", 1);
		SaveRelayForLifeTeams();
	}
	ABroadCast(COLOR_YELLOW, "BAO TRI: Streamer Plugin Shutting Down...", 1);
	...
	switch(frame)
	{
		case 1:
		{
			...
			if(IsPlayerMobile(playerid)) //Mobi
			{
				...
				PlayerTextDrawSetString(playerid, LoginMobi[playerid][7], string);
				for(new i = 0; i < 9; i++)
				{
					PlayerTextDrawShow(playerid, LoginMobi[playerid][i]);
					SelectTextDraw(playerid, 0xFFFFFFAA);
				}
			} else { //PC
				TimerLoading[playerid] = SetTimerEx("ShowPlayerLoading2", 100, true, "i", playerid);
			}
		}
		case 2:
		{
			...
			{
				format(string, sizeof(string), "%s", GetPlayerNameEx(playerid));
				PlayerTextDrawSetString(playerid, RegisterMobi[playerid][9], string);
				for(new i = 0; i < 12; i++)
				{
					PlayerTextDrawShow(playerid, RegisterMobi[playerid][i]);
					SelectTextDraw(playerid, 0xFFFFFFAA);
				}
			} else { //PC
				TimerLoading[playerid] = SetTimerEx("ShowPlayerLoading1", 300, true, "i", playerid);
			}
		}
		case 3:
		{
			format(titlestring, sizeof(titlestring), "GVRP - Dang nhap");
			format(string, sizeof(string), "{FFFF00}Mat khau khong dung hoac khong hop le{FFFFFF}\n\nTen tai khoan: {00FF00}%s{FFFFFF}, Dia chi IP: {0000FF}%s\n\n{FFFFFF}Tai khoan da duoc dang ky, vui long nhap mat khau de tien hanh dang nhap:", GetPlayerNameEx(playerid), GetPlayerIpEx(playerid), GetPlayerNameEx(playerid));
			ShowPlayerDialog(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Dang nhap","Thoat");
		}
		case 4:
		{
			format(titlestring, sizeof(titlestring), "Tai khoan dang tham gia - %s", GetPlayerNameEx(playerid));
    ...
    if(IsPlayerConnected(playerid))
	{
		...
		if(PlayerInfo[playerid][pTut] == 0)
		{
			...
			SetPlayerCameraLookAt(playerid, 2229.4968,-1722.0701,13.5625);
   			...
   			ShowPlayerDialog(playerid, REGISTERSEX, DIALOG_STYLE_LIST, "{FFFFFF}Nhan vat cua ban gioi tinh gi?", "Nam\nNu", "Lua chon", "");
			return 1;
		}
		new rand;
...
stock IsAContainerCar(carid)
{
	for (new v = 0; v < sizeof(ContainerVehicles); v++) {
	    if(carid == ContainerVehicles[v]) return 1;
	}
	return 0;
}
...
stock CreatelightPickup(model, type, Float:x, Float:y, Float:z, virtualworld = 0, color = 5 , type_color = 2)
{
    new lightcolor[4] = {19281, 19282, 19283, 19284};
    new lightcolor1[6] = {19289, 19290, 19291, 19292, 19293, 19294};
    new lightcolor2[4] = {19295, 19296, 19297, 19298};
    if(type_color == 0){
        if(-1 < color < 4) CreatePickup(lightcolor[color], type, x, y, z, virtualworld);
    }else if(type_color == 1){
        if(-1 < color < 6) CreatePickup(lightcolor1[color], type, x, y, z, virtualworld);
    }else{
        if(-1 < color < 4) CreatePickup(lightcolor2[color], type, x, y, z, virtualworld);
    }
...
forward OnCnChetTimerEnd(playerid);
public OnCnChetTimerEnd(playerid)
{
	SetPVarInt(playerid, "cnchettimer", 1);
    return 1;
}
...
forward ShowPlayerLoading(playerid);
public ShowPlayerLoading(playerid)
{
	new string[24];
	DangLoading[playerid] ++;
	if(DangLoading[playerid] == 5)
	{
		PlayerTextDrawShow(playerid, Loading[playerid][0]);
	}
	else if(DangLoading[playerid] == 20)
	{
		format(string, sizeof(string), "mdl-2001:load2");
		PlayerTextDrawSetString(playerid, Loading[playerid][1], string);
		PlayerTextDrawShow(playerid, Loading[playerid][1]);
	}
	else if(DangLoading[playerid] == 40)
	{
		format(string, sizeof(string), "mdl-2001:load3");
		PlayerTextDrawSetString(playerid, Loading[playerid][1], string);
		PlayerTextDrawShow(playerid, Loading[playerid][1]);
	}
	else if(DangLoading[playerid] == 60)
	{
		format(string, sizeof(string), "mdl-2001:load4");
		PlayerTextDrawSetString(playerid, Loading[playerid][1], string);
		PlayerTextDrawShow(playerid, Loading[playerid][1]);
	}
	else if(DangLoading[playerid] == 80)
	{
		format(string, sizeof(string), "mdl-2001:load5");
		PlayerTextDrawSetString(playerid, Loading[playerid][1], string);
		PlayerTextDrawShow(playerid, Loading[playerid][1]);
	}
	else if(DangLoading[playerid] == 110)
	{
		PlayerTextDrawHide(playerid, Loading[playerid][0]);
		PlayerTextDrawHide(playerid, Loading[playerid][1]);
		PlayerTextDrawHide(playerid, Loading[playerid][2]);
		KillTimer(TimerLoading[playerid]);
		DangLoading[playerid] = 0;
		format(string, sizeof(string), "%s", GetPlayerNameEx(playerid));
		PlayerTextDrawSetString(playerid, loginpanel_PTD[playerid][1], string);
		for(new i = 0; i < 5; i++)
		{
			PlayerTextDrawShow(playerid, loginpanel_PTD[playerid][i]);
			SelectTextDraw(playerid, 0xFFFFFFAA);
		}
	}
	return 1;
...
forward ShowPlayerLoading2(playerid);
public ShowPlayerLoading2(playerid)
{
	new string[24];
	DangLoading[playerid] ++;
	if(DangLoading[playerid] == 5)
...
forward ShowPlayerLoading1(playerid);
public ShowPlayerLoading1(playerid)
{
	new string[24];
	DangLoading[playerid] ++;
	if(DangLoading[playerid] == 5)
	...
	else if(DangLoading[playerid] == 110)
	{
		...
		DangLoading[playerid] = 0;
		format(string, sizeof(string), "%s", GetPlayerNameEx(playerid));
		PlayerTextDrawSetString(playerid, RegisterPC[playerid][5], string);
		for(new i = 0; i < 8; i++)
		{
			PlayerTextDrawShow(playerid, RegisterPC[playerid][i]);
...
stock ShowMenu(playerid)
{
	if(IsPlayerMobile(playerid)) //Mobi
	{
		for(new i = 0; i < 19; i++)
		{
			PlayerTextDrawShow(playerid, MenuMobi[playerid][i]);
			SelectTextDraw(playerid, 0x00FF00FF);
		}
		HideUpdate(playerid);
	...
	else //PC
	{
		for(new i = 0; i < 5; i++)
		{
			PlayerTextDrawShow(playerid, MenuPC[playerid][i]);
			SelectTextDraw(playerid, 0x00FF00FF);
		}
		for(new i = 0; i < 2; i++)
		{
			PlayerTextDrawHide(playerid, logoLSRP[playerid][i]);
		}
		ConnectingView(playerid);
	}
}

stock HideUpdate(playerid)
{
	for(new i = 10; i < 18; i++)
	{
...
stock HideCredits(playerid)
{
	for(new i = 0; i < 4; i++)
	{
		PlayerTextDrawHide(playerid, CreditsMobi[playerid][i]);
	}
}
...
stock ShowCredits(playerid)
{
	for(new i = 0; i < 4; i++)
	{
		PlayerTextDrawShow(playerid, CreditsMobi[playerid][i]);
	}
}
...
stock ShowShop(playerid)
{
	for(new i = 5; i < 16; i++)
	{
		PlayerTextDrawShow(playerid, ShopPC[playerid][i]);
	}
}
...
stock HideShop(playerid)
{
	for(new i = 5; i < 16; i++)
	{
		PlayerTextDrawHide(playerid, ShopPC[playerid][i]);
	}
}
...
stock ConnectingView(playerid)
{
    switch(random(10))
    {
        case 0, 2, 4, 6, 8, 10:
        {
            Streamer_UpdateEx(playerid, 1644.3745,-1722.6646,46.2533);
            InterpolateCameraPos(playerid,1644.3745,-1722.6646,46.2533, 2086.1904,-1790.4088,31.4953, 30000, CAMERA_MOVE);
            InterpolateCameraLookAt(playerid, 1986.9954,-1733.0861,27.0174, 1986.9954,-1733.0861,27.0174, 25000, CAMERA_MOVE);
        }
        case 1, 3, 5, 7, 9:
        {
            Streamer_UpdateEx(playerid, 2183.3169,-2347.4624,33.9568);
            InterpolateCameraPos(playerid, 2183.3169,-2347.4624,33.9568,2124.9363,-1991.8083,28.4478, 30000, CAMERA_MOVE);
            InterpolateCameraLookAt(playerid, 2224.5923,-2097.2917,34.4345, 2224.5923,-2097.2917,34.4345, 25000, CAMERA_MOVE);
        }
    }
    return 1;
}
		...
		if(GetPlayerVehicleID(playerid) != 0) {
		    if(TrollCar[playerid] != GetPlayerVehicleID(playerid))
			{
			    if(GetPVarInt(playerid, "KhongHacking") == 0 && GetPVarInt(playerid, "spam3") == 0)
			    {
	                ...
	                //==================//
	                new string[256];
					format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s (ID %d) da bi kick vi su dung hack troll car.", GetPlayerNameEx(playerid), playerid);
					ABroadCast(COLOR_LIGHTRED , string, 2);
			        KickEx(playerid);
					//==================//
				}
	    ...
	    if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && !IsAPlane(newcar) && !IsABoat(newcar))
		{
		    if(PlayerInfo[playerid][pAdmin] == 0)
		    {
		        if(GetSpeedKM(playerid) > TocDoGioiHan)
		        {
					...
					SendClientMessageEx(playerid, COLOR_GRAD1, "Ban dang su dung hacking speed car.");
					FlyCar[playerid] += 1;
					//====//
					new string[1280];
					format(string, sizeof(string), "[GVRP-AC] Player: %s (ID: %d) da bi kick vi hack speed car.", GetPlayerNameEx(playerid), playerid);
					ABroadCast(COLOR_LIGHTRED,string, 2);
					new stringz[1280];
					format(stringz, sizeof(stringz), "```[GVRP-AC] Player: %s (ID: %d) da bi kick vi hack speed car.```", GetPlayerNameEx(playerid), playerid);
					DCC_SendChannelMessage(CheckHackMess, stringz);
					KickEx(playerid);
				}
			}
		}
...
task CheckHackFly[1000]()
{
    foreach(new playerid : Player)
	{
		...
		if(pSpeed[playerid] >= 200.0 && PlayerInfo[playerid][pAdmin] == 0 && JetPack[playerid] == 0)
		{
			...
			{
			    format(string, sizeof(string), "[GVRP-AC] Player: %s (ID: %d) da bi kick vi hack slap .", GetPlayerNameEx(playerid), playerid,LaySpeedPlayerid(playerid));
			    ABroadCast(COLOR_LIGHTRED, string, 2);
			    new stringz[1280];
			    format(stringz, sizeof(stringz), "```[GVRP-AC] Player: %s (ID: %d) da bi kick vi hack slap.```", GetPlayerNameEx(playerid), playerid,LaySpeedPlayerid(playerid));
			    DCC_SendChannelMessage(CheckHackMess, stringz);
...
task AntiSkinCJ[1000]()
{
    foreach(new playerid : Player){
	    new mandeptrai;
	    mandeptrai = GetPlayerSkin(playerid);

		if(mandeptrai == 0 || mandeptrai == 92 || mandeptrai == 99)
		{
		    if(PlayerInfo[playerid][pAdmin] == 0){
			    new skin = random(311);
...
task AntiHackFly[1000]()
{
    foreach(new playerid : Player){
	    new man;
	    man = GetPlayerWeapon(playerid);

		if(GetSpeedPlayer(playerid) > 220 && man != 46)
		{
		    if(PlayerInfo[playerid][pAdmin] == 0 && GetPVarInt(playerid, "spam3") == 0)
		    {
                SetTimerEx("spam4", 5000, 0, "d", playerid);
				SetPVarInt( playerid, "spam3", 1);

		        new string[256];
			    format(string, sizeof(string), "[GVRP-AC] Player: %s (ID: %d) da bi kick vi hacking fly.", GetPlayerNameEx(playerid), playerid);
				ABroadCast(COLOR_LIGHTRED, string, 2);
				new stringz[256];
				format(stringz, sizeof(stringz), "```[GVRP-AC] Player: %s (ID: %d) da bi kick vi hacking fly.```", GetPlayerNameEx(playerid), playerid);
				DCC_SendChannelMessage(CheckHackMess, stringz);
				KickEx(playerid);
			}
		}
...
{
    CancelSelectTextDraw(playerid);
    SetPVarInt(playerid, "ExitVehicle", 0);
    for(new i = 0; i < 7; i++)
    {
...
stock SetPlayerOnline(playerid, stateid) // su dung pOnline de hien online cua player
{
	new string[128];
...
{
    if(range >= GetVehicleDistanceFromPoint(vehicleid, x, y, z)) return 1;
    return 0;
}
