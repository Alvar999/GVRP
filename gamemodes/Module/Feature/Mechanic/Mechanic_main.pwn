...
CMD:fix(playerid, params[])
{
    if(IsAMechanic(playerid))
	{
		if(IsPlayerInRangeOfPoint(playerid, 3.0, 1820.7505,-1434.9348,13.6016) || IsPlayerInRangeOfPoint(playerid, 3.0, 1820.6045,-1428.9097,13.6016))
		{
				...
				new closestcar = GetClosestCar(playerid);
	  			...
	  			if(IsPlayerInRangeOfVehicle(playerid, closestcar, 10.0))
	  			{
					if(!IsABike(closestcar) && !IsAPlane(closestcar)) {
						...
						{
							SendClientMessageEx(playerid, COLOR_GRAD1, "Nguoi choi chua mo mui xe (/car hood).");
							return 1;
						}
					}
	                ...
	                format(string, sizeof(string), "* %s da bat dau sua chua phuong tien.", GetPlayerNameEx(playerid));
	    			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
				}
				else return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong o gan mot chiec xe nao de sua chua.");
	  		}
	  	} 
	  	else return SendClientMessageEx(playerid, COLOR_YELLOW2,"Ban khong o diem sua xe");
    }
    else return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong phai la nguoi to chuc Mechanic." );
    return 1;
}
...
CMD:nos(playerid, params[])
{
    if(IsAMechanic(playerid)) {
        ...
        SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong phai la nguoi to chuc Mechanic." );
    }
    return 1;
}
