...
public OnPlayerLoad(playerid)
{
	...
	if(PlayerInfo[playerid][pReg] == 0)
	{
		...
		strcpy(PlayerInfo[playerid][pBirthDate], "1999-12-12", 64);
		...
		PlayerInfo[playerid][pFishermenSkill] = 0;
		PlayerInfo[playerid][pBuilderSkill] = 0;

		PlayerInfo[playerid][pRentBoat] = 0;
		...
		PlayerInfo[playerid][pKimCuong] = 0;

		PlayerInfo[playerid][pMayCua] = 0;
		...
		PlayerInfo[playerid][pMayKhoan] = 0;
		PlayerInfo[playerid][pScoin] = 0;
		PlayerInfo[playerid][pPhieuHouse] = 0;
	...
	for(new i = 0; i < 5; i++)
	{
		PlayerTextDrawHide(playerid, loginpanel_PTD[playerid][i]);
	}
	...
	{
		TextDrawShowForPlayer(playerid, Foods[0][playerid]);
		TextDrawShowForPlayer(playerid, Foods[1][playerid]);
		TextDrawShowForPlayer(playerid, Foods[2][playerid]);
		TextDrawShowForPlayer(playerid, Foods[3][playerid]);
	}
	TextDrawShowForPlayer(playerid, Foods_PC[playerid][0]);
	TextDrawShowForPlayer(playerid, Foods_PC[playerid][1]);
	TextDrawShowForPlayer(playerid, Foods_PC[playerid][2]);
	TextDrawShowForPlayer(playerid, Foods_PC[playerid][3]);
	ShowMenu(playerid);
	gPlayerLogged{playerid} = 1;
	g_mysql_AccountOnline(playerid, servernumber);
	GetHomeCount(playerid);

	TextDrawShowForPlayer(playerid, tdgvrp[0]);
	TextDrawShowForPlayer(playerid, tdgvrp[1]);
	new ip[32];
    ...
    new string128[1280], namsse[1280], version[1280], ipp[1280];
	GetPlayerVersion(playerid, version, sizeof(version));
    GetPlayerName(playerid,namsse,32);
    ...
    if(PlayerInfo[playerid][pVerified] != 0)
        email = "Da kich hoat";
	else email = "Chua kich hoat";
	
    format(string128,sizeof(string128),"[LOGIN-CONNECT]{FFFFFF} {ff9900}(Name: %s) - {ff9900}(IP: %s) - ({ff3300}ID: %d) - (Tinh trang: %s) (Client : %s).",namsse , ipp, playerid, email, version);
    ABroadCast(COLOR_YELLOW , string128, 2);
