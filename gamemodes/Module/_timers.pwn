...
////
timer LockSpeed[100](playerid)
{
	new Float:health, vehicleid = GetPlayerVehicleID(playerid);
	GetVehicleHealth(vehicleid, health);
	if(health <= 600)
	{
		LimitingVehicleSpeed(vehicleid, 60);
	}
	if(health <= 500)
	{
		LimitingVehicleSpeed(vehicleid, 50);
	}
	if(health <= 400)
	{
		LimitingVehicleSpeed(vehicleid, 40);
	}
	if(health <= 350)
	{
		LimitingVehicleSpeed(vehicleid, 30);
	}
	return 1;
}
...
task InjuredTimer[1000]()
{
	foreach(new i: Player)
	{
		if(GetPVarInt(i, "InjuredTimer") > 0)
		{
			SetPVarInt(i, "InjuredTimer", GetPVarInt(i, "InjuredTimer")-1);
		}
		else if(GetPVarInt(i, "InjuredTimer") < 0)
		{
			SetPVarInt(i, "InjuredTimer", 0);
		}
	}
}
...
//Giam sao - 5 phut
task DecreaseWantedLevel[300000]()
{
	foreach(new i: Player)
	{
		if(PlayerInfo[i][pWantedLevel] > 0)
		{
			...
			SendClientMessage(i, COLOR_RED, "Ban da duoc giam bot muc do truy na");
		}
	}
}
...
timer NOPCheck[5000](playerid)
{
    ...
    else if(IsAContainerCar(newcar) && PlayerInfo[playerid][pJob] != 33 && PlayerInfo[playerid][pJob2] != 33) ExecuteNOPAction(playerid);
    else if(IsAPizzaCar(newcar) && PlayerInfo[playerid][pJob] != 1 && PlayerInfo[playerid][pJob2] != 1) ExecuteNOPAction(playerid);
...
// TickRate: 10 Secs.
task ActorUpdate[10000]()
{
	UpdateActor();
	return 1;
}

task CuopBankUpdate[3000]()
{
	foreach(new i: Player)
	{
		if(PlayerInfo[i][pTVCuopNG] == 1 || PlayerInfo[i][pTVCuopNG] == 2)
		{
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		{
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Can ho tro? Doi ngu Junior Helpers co the giup do ban. (/yeucautrogiup de duoc giup do)");
		}
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se bi gioi han 2 gio choi dau cho nguoi moi tham gia cong dong GVRP.");
		}
	    ...
	    if(GetPVarType(i, "Injured"))
	    {
	        ...
	        if(GetPVarInt(i, "EMSAttempt") != 0)
			{
				...
				if(GetPVarInt(i, "EMSAttempt") == -1)
				{
					...
					new string[1280];
					format(string, sizeof(string), "(( Nguoi choi nay dang bi thuong ))");
                    SetPlayerChatBubble(i, string, TEAM_CYAN_COLOR, 50.0, 10000);
...
// Timer PayCheck
// 60s
task PayCheckTask[60000]()
{
	foreach(new i: Player)
	{
		PlayerInfo[i][pConnectSeconds] += 60;
		if(playerTabbed[i] == 0) {
			switch(PlayerInfo[i][pLevel]) {
				case 0 .. 2: PlayerInfo[i][pPayCheck] += Random(5, 6);
				case 3 .. 4: PlayerInfo[i][pPayCheck] += Random(5, 7);
				case 5 .. 6: PlayerInfo[i][pPayCheck] += Random(5, 8);
				case 7 .. 8: PlayerInfo[i][pPayCheck] += Random(5, 9);
				case 9 .. 10: PlayerInfo[i][pPayCheck] += Random(5, 10);
				case 11 .. 12: PlayerInfo[i][pPayCheck] += Random(5, 11);
				case 13 .. 14: PlayerInfo[i][pPayCheck] += Random(5, 12);
				case 15 .. 16: PlayerInfo[i][pPayCheck] += Random(5, 13);
				case 17 .. 18: PlayerInfo[i][pPayCheck] += Random(5, 14);
				case 19 .. 20: PlayerInfo[i][pPayCheck] += Random(5, 15);
				default: PlayerInfo[i][pPayCheck] += 10;
			}
	...
	foreach(new i: Player)
	{
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0) {
				...
				PhoneOnline[i] = 0;
				SendClientMessageEx(i, -1,"Lien he voi {FFFF00}Admin{FFFFFF} neu ban van bi ket trong tu");
				GameTextForPlayer(i, "~g~Ban da duoc tu do~n~~w~Chao mung ban quay tro lai voi the gioi", 5000, 1);
				ClearCrimes(i);
		...
		if(PlayerStoned[i] >= 3)
		{
			...
			if(PlayerStoned[i] == 50)
			{
				...
				SendClientMessageEx(i, COLOR_GRAD1, " Ban da het phe!");
			}
		}
		if(BoxWaitTime[i] > 0)
		{
		...
		if(PlayerCuffed[i] == 2)
		{
  		...
  		{
			new stringzz[1280];
			format(stringzz, sizeof(stringzz), "%d", PlayerInfo[i][pHunger]);
			TextDrawSetString(Foods[3][i], stringzz);
			TextDrawShowForPlayer(i, Foods[3][i]);
			new stringz[1280];
			format(stringz, sizeof(stringz),"%d", PlayerInfo[i][pKhatNuoc]);
			TextDrawSetString(Foods[2][i], stringz);
			TextDrawShowForPlayer(i, Foods[2][i]);
		}
		new stringzz[1280];
		format(stringzz, sizeof(stringzz), "%d", PlayerInfo[i][pHunger]);
		TextDrawSetString(Foods_PC[i][3], stringzz);
		TextDrawShowForPlayer(i, Foods_PC[i][3]);
		new stringz[1280];
		format(stringz, sizeof(stringz),"%d", PlayerInfo[i][pKhatNuoc]);
		TextDrawSetString(Foods_PC[i][2], stringz);
		TextDrawShowForPlayer(i, Foods_PC[i][2]);


		if (PlayerInfo[i][pHunger] == 50)
		{
			if(GetPVarInt(i, "Dangdemthoigian") == 0)
			{
				SetPVarInt(i, "Dangdemthoigian", 1);
				PlayerInfo[i][pHunger] = 49;
				SendClientMessageEx(i, COLOR_RED, "Ban dang bi doi vui long di an.");
				SetTimerEx("ThoiGianDoiBung", 10000, false, "i", i);
			}
		}
		if (PlayerInfo[i][pHunger] == 10)
		{
			PlayerInfo[i][pHunger] = 0;
			SendClientMessageEx(i, COLOR_RED, "Ban da chet vi doi");
			SetPlayerHealth(i, 0);
		}
		...
		// Khat Nuoc
		if (PlayerInfo[i][pKhatNuoc] == 50)
		{
			if(GetPVarInt(i, "Dangdemthoigian") == 0)
			{
				SetPVarInt(i, "Dangdemthoigian", 1);
				PlayerInfo[i][pKhatNuoc] = 49;
				SendClientMessageEx(i, COLOR_RED, "Ban dang bi khat vui long di uong.");
				SetTimerEx("ThoiGianDoiBung", 10000, false, "i", i);
			}
		}
		if(PlayerInfo[i][pKhatNuoc] == 5)
		{
			PlayerInfo[i][pKhatNuoc] = 0;
			SendClientMessageEx(i, COLOR_RED, "Ban da chet vi khat");
			SetPlayerHealth(i, 0);
		}
...
task ServerHeartbeatTwo[1000]() {
	...
	foreach(new i: Player)
	{
		...
		// if(PlayerInfo[i][pNation] == -1 && TutStep[i] == 24 && RegistrationStep[i] == 3)
		// {
		// 	//NationSel_HandleNationSelection(i);
		// 	ShowPlayerDialog(i, DIALOG_NATION, DIALOG_STYLE_LIST, "Chon quoc tich", "San Andreas\nTierra Robada", "Chon", "");
		// }
    ...
    for(new v = 0; v < MAX_VEHICLES; v++) if(GetVehicleModel(v) && GetVehicleModel(v) != 481 && GetVehicleModel(v) != 509 && GetVehicleModel(v) != 510) {
	    ...
	    if(engine == VEHICLE_PARAMS_ON) {
			if(arr_Engine{v} == 0) SetVehicleParamsEx(v,VEHICLE_PARAMS_OFF,lights,alarm,doors,bonnet,boot,objective);
				...
				foreach(new i: Player)
				{
					if(VehicleFuel[v] <= 10.0) SendClientMessage(v, -1, "{ffff00}Canh bao:{ffffff} Phuong tien cua ban sap het xang!!!");
				}
