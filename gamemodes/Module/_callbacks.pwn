	...
	if(weaponid != 38 && weaponid != 28 && weaponid != 32)
    {
        ...
        if(Rapid[playerid] > 15)
        {
            if(PlayerInfo[playerid][pAdmin] == 0) {
	            new string[256];
				format(string, sizeof(string), "[GVRP-AC] Player: %s (ID: %d) co the dang su dung hacking speed fire.", GetPlayerNameEx(playerid), playerid);
...
{
	defer LockSpeed(playerid);
	if(PlayerInfo[playerid][pHunger] >= 101) {
		PlayerInfo[playerid][pHunger] = 100;
	}
	if(PlayerInfo[playerid][pHunger] <= 0) {
		PlayerInfo[playerid][pHunger] = 0;
	}
	if(PlayerInfo[playerid][pKhatNuoc] >= 101) {
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	if(clickedid == Text:INVALID_TEXT_DRAW)
	{
		...
		Player_Notice(playerid, AUTO_MESSAGE, "Ban da dong giao dien tui do");
	}
	if(Text:INVALID_TEXT_DRAW == clickedid) {
	    if(GetPVarInt(playerid, "ExitVehicle") == 1) {
	        ExitVehiclemod(playerid);
		}
    }
	return 0;
}
...
{
	if(playertextid == VehicleMod_PC[playerid][5])
	{
		new string[128];
	...
	if(playertextid == VehicleMod_PC[playerid][1])
	{
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
	...
	if(playertextid == VehicleMod_PC[playerid][4])
	{
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	...
	if(playertextid == VehicleMod_PC[playerid][2])
	{
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			new vehicleid = GetPlayerVehicleID(playerid);
	...
	if(playertextid == VehicleMod_PC[playerid][3])
	{
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			cmd_pvlock(playerid,"");
	...
	if(playertextid == VehicleMod_PC[playerid][6])
	{
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			cmd_timxe(playerid,"");
	...
	if(playertextid == tdtuido1[playerid][4])
	{
		hidetuido(playerid);
		CancelSelectTextDraw(playerid);
		Player_Notice(playerid, AUTO_MESSAGE, "Ban da dong giao dien tui do");
	...
	if(playertextid == tdtuido1[playerid][9])
	{
		...
		{
			ShowPlayerDialog(playerid, DIALOG_REPORTMENU, DIALOG_STYLE_LIST, "Report Menu [1/2]", "Giet nguoi ko ly do\nbi dung yen\nHacking\nChicken Running\nCar Ramming\nPower Gaming\nMeta Gaming\nGun Discharge Exploits (QS/CS)\nSpamming\nMoney Farming\nBan Evader\nGeneral Exploits\nReleasing Hitman Names\nRunning Man Exploiter\nCar Surfing\nNonRP Behavior\nNext Page","Select", "Exit");
		}
		hidetuido(playerid);
		CancelSelectTextDraw(playerid);
		Player_Notice(playerid, AUTO_MESSAGE, "Ban da dong giao dien tui do");
	...
	if(playertextid == tdtuido1[playerid][11])
	{
		Player_Notice(playerid, AUTO_MESSAGE, "Se cap nhat sau");
	}

	if(playertextid == tdtuido4[playerid][66])
	{
		SetPVarInt(playerid, "selection", 1);
	}

	if(playertextid == tdtuido4[playerid][67])
	{
		SetPVarInt(playerid, "selection", 2);
	}

	if(playertextid == tdtuido4[playerid][68])
	{
		SetPVarInt(playerid, "selection", 3);
	}

	if(playertextid == tdtuido4[playerid][69])
	{
		SetPVarInt(playerid, "selection", 4);
	}

	if(playertextid == tdtuido4[playerid][70])
	{
		SetPVarInt(playerid, "selection", 5);
	}

	if(playertextid == tdtuido4[playerid][71])
	{
		SetPVarInt(playerid, "selection", 6);
	}

	if(playertextid == tdtuido4[playerid][72])
	{
		SetPVarInt(playerid, "selection", 7);
	}

	if(playertextid == tdtuido4[playerid][73])
	{
		SetPVarInt(playerid, "selection", 8);
	}

	if(playertextid == tdtuido4[playerid][74])
	{
		SetPVarInt(playerid, "selection", 9);
	}

	if(playertextid == tdtuido6[playerid][103])
	{
		SetPVarInt(playerid, "selection", 10);
	}

	if(playertextid == tdtuido4[playerid][76])
	{
		SetPVarInt(playerid, "selection", 11);
	}

	if(playertextid == tdtuido6[playerid][105])
	{
		SetPVarInt(playerid, "selection", 12);
	}

	if(playertextid == tdtuido4[playerid][78])
	{
		SetPVarInt(playerid, "selection", 13);
	}

	if(playertextid == tdtuido4[playerid][79])
	{
		SetPVarInt(playerid, "selection", 14);
	}

	if(playertextid == tdtuido4[playerid][80])
	{
		SetPVarInt(playerid, "selection", 15);
	}

	if(playertextid == tdtuido5[playerid][81])
	{
		SetPVarInt(playerid, "selection", 16);
	}

	if(playertextid == tdtuido5[playerid][82])
	{
		SetPVarInt(playerid, "selection", 17);
	}

	if(playertextid == tdtuido5[playerid][83])
	{
		SetPVarInt(playerid, "selection", 18);
	}

	if(GetPVarInt(playerid, "selection") != 0)
	{
		if(playertextid == tdtuido1[playerid][3])
		{
			if(GetPVarInt(playerid, "selection") == 1)
			{
				if(PlayerInfo[playerid][piDE] > 0)
				{
					GivePlayerValidWeapon(playerid, 24, 1);
					PlayerInfo[playerid][piDE] = 0;
					SetPVarInt(playerid, "selection", 0);
                	Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~DE~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 2)
			{
				if(PlayerInfo[playerid][piMp5] > 0)
				{
					GivePlayerValidWeapon(playerid, 29, 1);
					PlayerInfo[playerid][piMp5] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~MP5~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 3)
			{
				if(PlayerInfo[playerid][piAk] > 0)
				{
					GivePlayerValidWeapon(playerid, 30, 1);
					PlayerInfo[playerid][piAk] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~Ak47~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 4)
			{
				if(PlayerInfo[playerid][piCombat] > 0)
				{
					GivePlayerValidWeapon(playerid, 27, 1);
					PlayerInfo[playerid][piCombat] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~Combat Shotgun~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 5)
			{
				if(PlayerInfo[playerid][piM4] > 0)
				{
					GivePlayerValidWeapon(playerid, 31, 1);
					PlayerInfo[playerid][piM4] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~M4a1~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 6)
			{
				if(PlayerInfo[playerid][piSniper] > 0)
				{
					GivePlayerValidWeapon(playerid, 34, 1);
					PlayerInfo[playerid][piSniper] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~Sniper~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 7)
			{
				if(PlayerInfo[playerid][piShotgun] > 0)
				{
					GivePlayerValidWeapon(playerid, 25, 1);
					PlayerInfo[playerid][piShotgun] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~Shotgun~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 8)
			{
				if(PlayerInfo[playerid][piTec9] > 0)
				{
					GivePlayerValidWeapon(playerid, 32, 1);
					PlayerInfo[playerid][piTec9] = 0;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da lay mot cay ~r~Tec9~w~ khoi tui do");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 9)
			{
				if(PlayerInfo[playerid][pCanXang] > 0)
				{
					PlayerInfo[playerid][pXang] = PlayerInfo[playerid][pMayKhoan] * 100;
					PlayerInfo[playerid][pCanXang] -= 1;
					SetPVarInt(playerid, "selection", 0);
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da do ~r~Xang~w~ cho may khoan");
				}
			}

			else if(GetPVarInt(playerid, "selection") == 10)
			{
				if(PlayerInfo[playerid][pMayKhoan] > 0)
				{
					Player_Notice(playerid, AUTO_MESSAGE, "Ban dang co mot ~r~may khoan thuong");
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 11)
			{
				if(PlayerInfo[playerid][pMayKhoan] > 1)
				{
					Player_Notice(playerid, AUTO_MESSAGE, "Ban dang co mot ~r~may khoan vip");
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 12)
			{
				if(PlayerInfo[playerid][pKimCuong] > 0)
				{
					new string1[128];
					format(string1, sizeof(string1),"Ban dang co ~r~%d kim cuong~w~ trong tui do", PlayerInfo[playerid][pKimCuong]);
					Player_Notice(playerid, AUTO_MESSAGE, string1);
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 13)
			{
				if(PlayerInfo[playerid][pDirtyMoney] > 0)
				{
					new string0[128];
					format(string0, sizeof(string0),"Ban dang co ~r~%d Tien ban~w~ trong tui do", PlayerInfo[playerid][pDirtyMoney]);
					Player_Notice(playerid, AUTO_MESSAGE, string0);
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 14)
			{
				if(PlayerInfo[playerid][pWeed] > 0)
				{
					new string2[128];
					format(string2, sizeof(string2),"Ban dang co ~r~%d Can sa~w~ trong tui do", PlayerInfo[playerid][pWeed]);
					Player_Notice(playerid, AUTO_MESSAGE, string2);
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 15)
			{
				if(PlayerInfo[playerid][pCrom] > 0)
				{
					new string3[128];
					format(string3, sizeof(string3),"Ban dang co ~r~%d Crom~w~ trong tui do", PlayerInfo[playerid][pCrom]);
					Player_Notice(playerid, AUTO_MESSAGE, string3);
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 16)
			{
				if(PlayerInfo[playerid][pRuteni] > 0)
				{
					new string4[128];
					format(string4, sizeof(string4),"Ban dang co ~r~%d Ruteni~w~ trong tui do", PlayerInfo[playerid][pRuteni]);
					Player_Notice(playerid, AUTO_MESSAGE, string4);
					SetPVarInt(playerid, "selection", 0);
				}
			}

			else if(GetPVarInt(playerid, "selection") == 17)
			{
				if(PlayerInfo[playerid][pTitan] > 0)
				{
					new string5[128];
					format(string5, sizeof(string5),"Ban dang co ~r~%d Titan~w~ trong tui do", PlayerInfo[playerid][pTitan]);
					Player_Notice(playerid, AUTO_MESSAGE, string5);
					SetPVarInt(playerid, "selection", 0);
				}
			}
			...
			else if(GetPVarInt(playerid, "selection") == 16)
			{
				...
				{
					new string6[128];
					format(string6, sizeof(string6),"Ban dang co ~r~%d Wolfram~w~ trong tui do", PlayerInfo[playerid][pWolfram]);
					Player_Notice(playerid, AUTO_MESSAGE, string6);
					SetPVarInt(playerid, "selection", 0);
				}
			}
	...
	if(playertextid == MenuMobi[playerid][2]) //Shop Mobi
	{
		HideCredits(playerid);
		HideUpdate(playerid);
		ShowPlayerDialog(playerid, DIALOG_SHOPMOBI, DIALOG_STYLE_TABLIST_HEADERS, "SHOP",
		"STT\tTen san pham\tGia KC\n\
		1\tCar Voucher\t450\n\
		2\tMoney 2m\t100\n\
		3\tVIP GOLD\t300\n\
		4\tVIP SILVER\t150\n\
		...
		6\tMay khoan VIP\t100\n\
		7\tRimkit\t100\n\
		8\tMay khoan thuong\t0",
		"Mua", "Huy bo");
	}
	if(playertextid == MenuMobi[playerid][3]) //Cap nhat Mobi
	{
		new string[256];
		format(string, sizeof(string), "1. Cong viec: Cong nhan, Tho moc, Giao hang, Pizza Boy, Danh ca hien da duoc cap nhat moi nhat trong may chu.");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][10], string);
		format(string, sizeof(string), "2. He thong dang nhap voi mot giao dien moi cho PC va Android.");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][11], string);
		format(string, sizeof(string), "3. Dang cap nhat.");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][12], string);
		format(string, sizeof(string), "4. Dang cap nhat.");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][13], string);
		
		HideCredits(playerid);
		ShowUpdate(playerid);
	}
	if(playertextid == MenuMobi[playerid][4]) //Su kien Mobi
	{
		new string[256];
		format(string, sizeof(string), "1. Su kien Fam kim cuong doi lay tien mat cho cac loai cong viec trong may chu");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][10], string);
		format(string, sizeof(string), "2. Dang cap nhat");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][11], string);
		format(string, sizeof(string), "3. Dang cap nhat.");
		PlayerTextDrawSetString(playerid, MenuMobi[playerid][12], string);
		format(string, sizeof(string), "4. Dang cap nhat.");
	...
	if(playertextid == MenuMobi[playerid][5]) //Vao Game
	{
		HideCredits(playerid);
		HideUpdate(playerid);
		PlayerTextDrawHide(playerid, BGLOGIN[playerid][0]);
		for(new i = 0; i < 19; i++)
		{
			PlayerTextDrawHide(playerid, MenuMobi[playerid][i]);
		}
		CancelSelectTextDraw(playerid);
		SetCameraBehindPlayer(playerid);
		TogglePlayerSpectating(playerid, 0);
	}
	if(playertextid == ShopPC[playerid][6]) //GVIP
	{
		new string[24];
		SetPVarInt(playerid, "MuaGVIP", 1);
		format(string, sizeof(string), "mdl-2004:gvip");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
		DeletePVar(playerid, "MuaCAR");
		DeletePVar(playerid, "MuaDOLLAR");
		DeletePVar(playerid, "MuaRIMKIT");
		DeletePVar(playerid, "MuaMK");
		DeletePVar(playerid, "MuaMKVIP");
	}
	if(playertextid == ShopPC[playerid][7]) //SVIP
	{
		new string[24];
		SetPVarInt(playerid, "MuaSVIP", 1);
		format(string, sizeof(string), "mdl-2004:svip");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaGVIP");
		DeletePVar(playerid, "MuaHouse");
		DeletePVar(playerid, "MuaCAR");
		DeletePVar(playerid, "MuaDOLLAR");
		DeletePVar(playerid, "MuaRIMKIT");
	...
	if(playertextid == ShopPC[playerid][8]) //HOUSE
	{
		new string[24];
		SetPVarInt(playerid, "MuaHouse", 1);
		format(string, sizeof(string), "mdl-2004:house");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaGVIP");
	...
	if(playertextid == ShopPC[playerid][9]) //CAR
	{
		new string[24];
		SetPVarInt(playerid, "MuaCAR", 1);
		format(string, sizeof(string), "mdl-2004:car");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
	...
	if(playertextid == ShopPC[playerid][10]) //DOLLAR
	{
		new string[24];
		SetPVarInt(playerid, "MuaDOLLAR", 1);
		format(string, sizeof(string), "mdl-2004:dollar");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
	...
	if(playertextid == ShopPC[playerid][11]) //RIMKIT
	{
		new string[24];
		SetPVarInt(playerid, "MuaRIMKIT", 1);
		format(string, sizeof(string), "mdl-2004:rimkit");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
	...
	if(playertextid == ShopPC[playerid][12]) //MAYKHOAN
	{
		new string[24];
		SetPVarInt(playerid, "MuaMK", 1);
		format(string, sizeof(string), "mdl-2004:mk");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
	...
	if(playertextid == ShopPC[playerid][13]) //MAYKHOANVIP
	{
		new string[24];
		SetPVarInt(playerid, "MuaMKVIP", 1);
		format(string, sizeof(string), "mdl-2004:mkvip");
		PlayerTextDrawSetString(playerid, ShopPC[playerid][14], string);
		PlayerTextDrawShow(playerid, ShopPC[playerid][14]);
		DeletePVar(playerid, "MuaSVIP");
		DeletePVar(playerid, "MuaHouse");
	...
	if(playertextid == ShopPC[playerid][15]) //MUA
	{
		if(GetPVarInt(playerid, "MuaGVIP") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 300)
			{
				PlayerInfo[playerid][pKimCuong] -= 300;
				PlayerInfo[playerid][pGVIPVoucher] += 1;
				Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~VIP GOLD ~w~voi gia ~r~300 kim cuong - /myvouchers de su dung");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~VIP GOLD");
		}
		else if(GetPVarInt(playerid, "MuaSVIP") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 150)
			{
				PlayerInfo[playerid][pKimCuong] -= 150;
				PlayerInfo[playerid][pSVIPVoucher] += 1;
				Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~VIP SILVER ~w~voi gia ~r~150 kim cuong - /myvouchers de su dung");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~VIP SILVER");
		}
		else if(GetPVarInt(playerid, "MuaHouse") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 300)
			{
				PlayerInfo[playerid][pKimCuong] -= 300;
				PlayerInfo[playerid][pPhieuHouse] += 1;
				Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~Phieu HOUSE ~w~voi gia ~r~300 kim cuong");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~Phieu HOUSE");
		}
		else if(GetPVarInt(playerid, "MuaCAR") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 450)
			{
				/*PlayerInfo[playerid][pKimCuong] -= 450;
				PlayerInfo[playerid][pVehVoucher] += 1;*/
				Player_Notice(playerid, AUTO_MESSAGE, "Chuc nang tam khoa");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua phieu ~g~CAR");
		}
		else if(GetPVarInt(playerid, "MuaDOLLAR") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 100)
			{
				PlayerInfo[playerid][pKimCuong] -= 100;
				PlayerInfo[playerid][pCash] += 2000000;
				Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~2M MONEY ~w~voi gia ~r~100 kim cuong");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~2M MONEY");
		}
		else if(GetPVarInt(playerid, "MuaRIMKIT") == 1)
		{
			if(PlayerInfo[playerid][pKimCuong] >= 100)
			{
				PlayerInfo[playerid][pKimCuong] -= 100;
				PlayerInfo[playerid][pRimMod] += 1;
				Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~RIMKIT ~w~voi gia ~r~100 kim cuong");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~RIMKIT");
		}
		else if(GetPVarInt(playerid, "MuaMK") == 1)
		{
			if(PlayerInfo[playerid][pScoin] >= 0)
			{
				if(PlayerInfo[playerid][pMayKhoan] < 1)
				{
					PlayerInfo[playerid][pScoin] = 0;
					PlayerInfo[playerid][pMayKhoan] = 1;
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~MAY KHOAN ~w~voi gia ~r~0 Scoin");
				}
				else Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~MAY KHOAN");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~MAY KHOAN");
		}
		else if(GetPVarInt(playerid, "MuaMKVIP") == 1)
		{
			if(PlayerInfo[playerid][pScoin] >= 100)
			{
				if(PlayerInfo[playerid][pMayKhoan] < 2)
				{
					PlayerInfo[playerid][pScoin] -= 100;
					PlayerInfo[playerid][pMayKhoan] = 2;
					Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~MAY KHOAN VIP ~w~voi gia ~r~100 Scoin");
				}
				else Player_Notice(playerid, AUTO_MESSAGE, "Ban da mua ~g~MAY KHOAN VIP");
			}
			else Player_Notice(playerid, AUTO_MESSAGE, "Ban khong du ~r~kim cuong ~w~de mua ~g~MAY KHOAN VIP");
		}
	}
	if(playertextid == MenuPC[playerid][0]) //Shop
	{
		ShowShop(playerid);
		for(new i = 5; i < 8; i++)
		{
			PlayerTextDrawHide(playerid, Event[playerid][i]);
		}
	}
	if(playertextid == MenuPC[playerid][1]) //Update
	{
		new string[32];
		format(string, sizeof(string), "mdl-2006:update");
		PlayerTextDrawSetString(playerid, Event[playerid][5], string);
		HideShop(playerid);
		for(new i = 5; i < 8; i++)
		{
			PlayerTextDrawShow(playerid, Event[playerid][i]);
		}
	}
	if(playertextid == MenuPC[playerid][2]) //Event
	{
		new string[32];
		format(string, sizeof(string), "mdl-2005:event1");
		PlayerTextDrawSetString(playerid, Event[playerid][5], string);
		HideShop(playerid);
		for(new i = 5; i < 8; i++)
		{
	...
	if(playertextid == MenuPC[playerid][3]) //Credits
	{
		new string[32];
		format(string, sizeof(string), "mdl-2007:credits");
		PlayerTextDrawSetString(playerid, Event[playerid][5], string);
		HideShop(playerid);
		for(new i = 5; i < 8; i++)
		{
	...
	if(playertextid == MenuPC[playerid][4]) //Start
	{
		HideShop(playerid);
		DeletePVar(playerid, "MuaMKVIP");
		DeletePVar(playerid, "MuaSVIP");
		...
		DeletePVar(playerid, "MuaMK");
		DeletePVar(playerid, "MuaGVIP");
		PlayerTextDrawHide(playerid, logoLSRP[playerid][0]);
		PlayerTextDrawHide(playerid, logoLSRP[playerid][1]);
		for(new i = 5; i < 8; i++)
		{
			PlayerTextDrawHide(playerid, Event[playerid][i]);
		}
		for(new i = 0; i < 5; i++)
		{
			PlayerTextDrawHide(playerid, MenuPC[playerid][i]);
		}
		CancelSelectTextDraw(playerid);
		SetCameraBehindPlayer(playerid);
		TogglePlayerSpectating(playerid, 0);
	...
	if(playertextid == FoodShop[playerid][3])
	{
		for(new i = 0; i < 13; i++)
		{
			PlayerTextDrawHide(playerid, FoodShop[playerid][i]);
			CancelSelectTextDraw(playerid);
			DeletePVar(playerid, "SelectFood");
		}
	}
	if(playertextid == FoodShop[playerid][4])
	{
		if(PlayerInfo[playerid][pCash] >= 10000)
		{
			if(PlayerInfo[playerid][pHunger] < 100 || PlayerInfo[playerid][pKhatNuoc] < 100)
			{
				PlayerInfo[playerid][pHunger] += 40;
				PlayerInfo[playerid][pKhatNuoc] += 40;
				PlayerInfo[playerid][pCash] -= 10000;
				SendClientMessageEx(playerid, -1, "{FF0000}[!] {FFFFFF}Ban da mua Ga ran + Nuoc ngot voi gia $10,000");
			}
			else SendClientMessageEx(playerid, -1, "{FF0000}[?] {FFFFFF}Ban da no co the khong the tiep thu them thuc an duoc nua!");
		}
	}
	if(playertextid == FoodShop[playerid][5])
	{
		if(PlayerInfo[playerid][pCash] >= 10000)
		{
			if(PlayerInfo[playerid][pHunger] < 100 || PlayerInfo[playerid][pKhatNuoc] < 100)
			{
				...
				{			
					...
					PlayerInfo[playerid][pKhatNuoc] += 40;
					PlayerInfo[playerid][pCash] -= 10000;
					SendClientMessageEx(playerid, -1, "{FF0000}[!] {FFFFFF}Ban da mua Pizza + Nuoc ngot voi gia $10,000");
				}
			}
			else SendClientMessageEx(playerid, -1, "{FF0000}[?] {FFFFFF}Ban da no co the khong the tiep thu them thuc an duoc nua!");
		}
	}
	if(playertextid == FoodShop[playerid][6])
	{
		if(PlayerInfo[playerid][pCash] >= 10000)
		{
			if(PlayerInfo[playerid][pHunger] < 100 || PlayerInfo[playerid][pKhatNuoc] < 100)
			{
				if(PlayerInfo[playerid][pKhatNuoc] < 100)
				{
					...
					PlayerInfo[playerid][pKhatNuoc] += 40;
					PlayerInfo[playerid][pCash] -= 10000;
					SendClientMessageEx(playerid, -1, "{FF0000}[!] {FFFFFF}Ban da mua Burger + Nuoc ngot voi gia $10,000");
				}
				else SendClientMessageEx(playerid, -1, "{FF0000}[?] {FFFFFF}Ban da du nuoc khong the tiep thu them thuc an duoc nua!");
			}
			else SendClientMessageEx(playerid, -1, "{FF0000}[?] {FFFFFF}Ban da no co the khong the tiep thu them thuc an duoc nua!");
		}
	}
	if(playertextid == RegisterPC[playerid][4])
	{
		if(GetPVarInt(playerid, "DaNhapEmail") == 1)
		{
			if(GetPVarInt(playerid, "DaNhapMK") == 1)
			{
				new string[256], szBuffer[64], szEmail1[64], query[356];	
				new passbuffer[129];
				GetPVarString(playerid, "MatKhau", szBuffer, sizeof(szBuffer));
				WP_Hash(passbuffer, sizeof(passbuffer), szBuffer);

				GetPVarString(playerid, "NhapEmail", szEmail1, sizeof(szEmail1));
				format(query, sizeof(query), "UPDATE `accounts` SET `Email`='%s' WHERE `id`=%d", szEmail1, GetPlayerSQLId(playerid));
				...
				for(new o = 0; o < 8; o++)
				{
					PlayerTextDrawHide(playerid, RegisterPC[playerid][o]);
				}
				CancelSelectTextDraw(playerid);
				DeletePVar(playerid, "DaNhapMK");
				DeletePVar(playerid, "DaNhapEmail");
			}
			else SendClientMessageEx(playerid, -1, "Ban chua nhap mat khau nen khong the dang ky");
		}
		else SendClientMessageEx(playerid, -1, "Ban chua nhap Email nen khong the dang ky");
	}
	if(playertextid == RegisterMobi[playerid][2])
	{
		if(GetPVarInt(playerid, "DaNhapEmail") == 1)
		{
			if(GetPVarInt(playerid, "DaNhapMK") == 1)
			{
				...
				for(new o = 0; o < 12; o++)
				{
					PlayerTextDrawHide(playerid, RegisterMobi[playerid][o]);
				}
				CancelSelectTextDraw(playerid);
				DeletePVar(playerid, "DaNhapMK");
				DeletePVar(playerid, "DaNhapEmail");
	...
	if(playertextid == RegisterPC[playerid][2])
	{
		new string[1280];
		format(string, sizeof(string),"Tai khoan %s chua duoc dang ky\nNhap mat khau vao day de vao ame");
		ShowPlayerDialog(playerid, DIALOG_NHAPMATKHAU1, DIALOG_STYLE_INPUT, "Nhap mat khau", string, "Xac Nhan", "< ");
	}
	if(playertextid == RegisterPC[playerid][3])
	{
		ShowPlayerDialog(playerid, DIALOG_NHAPEMAIL, DIALOG_STYLE_INPUT, "Nhap Email", "Ban hay nhap Email vao day:", "Xong", "Huy bo");
	}
	if(playertextid == RegisterMobi[playerid][5])
	{
		ShowPlayerDialog(playerid, DIALOG_NHAPMATKHAU1, DIALOG_STYLE_INPUT, "Nhap mat khau", "Ban hay nhap mat khau vao day:", "Xong", "Huy bo");
	}
	if(playertextid == RegisterMobi[playerid][7])
	{
		ShowPlayerDialog(playerid, DIALOG_NHAPEMAIL, DIALOG_STYLE_INPUT, "Nhap Email", "Ban hay nhap Email vao day:", "Xong", "Huy bo");
	}
	if(playertextid == loginpanel_PTD[playerid][2])
	{
		new string[1280], stringg[1280];
		format(string, sizeof(string),"{7c7872}Tai khoan{FFFFFF} {ffc103}%s{FFFFFF} {7c7872}da duoc {caad52}dang ky{FFFFFF}\n{7c7872}Nhap mat khau de tiep tuc.", GetPlayerNameEx(playerid));
		format(stringg, sizeof(stringg),"{7c7872}Login-Name: {caad52}%s{FFFFFF}", GetPlayerNameEx(playerid));
		ShowPlayerDialog(playerid, DIALOG_NHAPMATKHAU, DIALOG_STYLE_PASSWORD, stringg, string, "Xac Nhan", "<");
	}
	if(playertextid == LoginMobi[playerid][5])
	{
		ShowPlayerDialog(playerid, DIALOG_NHAPMATKHAU, DIALOG_STYLE_INPUT, "Nhap mat khau", "Ban hay nhap mat khau vao day:", "Xong", "Huy bo");
	}
	if(playertextid == LoginMobi[playerid][2])
	{
		if(GetPVarInt(playerid, "DaNhapMK") == 1){
			new string[128];
		...
		else SendClientMessageEx(playerid, -1, "Ban chua nhap mat khau nen khong the dang nhap");
	}
	if(playertextid == loginpanel_PTD[playerid][3])
	{
		if(GetPVarInt(playerid, "DaNhapMK") == 1){
			new string[128];
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid)
{
    ...
    if(weaponid == 0)
        format(stringdz, sizeof(stringdz), "Ban da dung {ff9900}Unarmed{FFFFFF} gay {ff9900}%.2f{FFFFFF} cho {ff9900}%s{FFFFFF} (%0.2fm)", amount,  victim, Distance);
    else
        format(stringdz, sizeof(stringdz), "Ban da dung {ff9900}%s{FFFFFF} gay {ff9900}%.2f{FFFFFF} cho {ff9900}%s{FFFFFF} (%0.2fm)", weaponname, amount,  victim, Distance);
    ...
    if(weaponid == 0)
         format(infoString, sizeof(infoString), "{ff9900}%s{FFFFFF} da su dung {ff9900}Unarmed{FFFFFF} gay cho ban {ff9900}%.2f{FFFFFF} (%0.2fm)", attackerName, amount,  Distance);
     else
        format(infoString, sizeof(infoString), "{ff9900}%s{FFFFFF} da su dung {ff9900}%s{FFFFFF} gay cho ban {ff9900}%.2f{FFFFFF} (%0.2fm)", attackerName, weaponName, amount,  Distance);
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(!ispassenger)
	{
		...
		if(IsWeaponizedVehicle(GetVehicleModel(vehicleid)))
	    {
	        if(PlayerInfo[playerid][pLevel] != 0)
	        {
				...
				{
					new szString[128];
					format(szString, sizeof(szString), "[GVRP-AC]: %s (ID: %d) co the dang hack Troll Car", GetPlayerNameEx(playerid), playerid);
		...
		else if(IsATruckerCar(vehicleid))
		{
		    if((PlayerInfo[playerid][pJob] == 16 || PlayerInfo[playerid][pJob2] == 16))
			{
				...
				{
    				SendClientMessageEx(playerid, COLOR_YELLOW, "TRUCKER JOB: De co duoc hang hoa,su dung /layhang cho tai xe.");
				}
...
public OnPlayerConnect(playerid) {
	...
	SetPVarInt(playerid, "MoiCUOPNH", INVALID_PLAYER_ID);
	...
	PlayerInfo[playerid][pVIPExpire] < gettime();
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(gPlayerLogged{playerid} == 1)
	{
			...
			if(hgActive == 2)
			{
				if(hgPlayerCount == 3)
				{
					...
					for(new w = 0; w < 12; w++)
					{
						...
						{
							GivePlayerValidWeapon(playerid, PlayerInfo[playerid][pGuns][w], 1);
						}
					}
				}
					...
					for(new i = 0; i < MAX_PLAYERS; i++)
					{
						if(HungerPlayerInfo[i][hgInEvent] == 1)
						{
							...
							for(new w = 0; w < 12; w++)
							{
								...
								{
									GivePlayerValidWeapon(i, PlayerInfo[i][pGuns][w], 1);
								}
							}
						}
					}
		...
		if(gettime() >= PlayerInfo[playerid][pCnChetTime]) PlayerInfo[playerid][pCnChetTime] = 0;
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(IsAMedic(playerid) && PlayerInfo[playerid][pDuty]) {
		...
		SendClientMessageEx (killerid, COLOR_RED, "Ban da giet nguoi cua chinh phu ban bi truy na 2 sao");
	}
	if(IsACop(playerid) && PlayerInfo[playerid][pDuty]) {
		GetPVarInt(playerid, "IsInArena");
		PlayerInfo[killerid][pWantedLevel] += 2;
		SetPlayerWantedLevel(killerid, PlayerInfo[killerid][pWantedLevel]);
		SendClientMessage (killerid, COLOR_RED, "Ban da giet nguoi cua chinh phu ban bi truy na 2 sao");
	}
    if(IsPlayerNPC(playerid)) return 1;
	IsSpawned[playerid] = 0;
	SpawnKick[playerid] = 0;
...
{	
	LimitingVehicleSpeed(596, 150);

	IsPlayerHaveAutoaim(playerid);
...
public OnPlayerEnterCheckpoint(playerid)
{
	if(CP[playerid] == 252000)
 	{
	 	...
	 	DisablePlayerCheckpoint(playerid);
	 	SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da den vi tri duoc danh dau!.");
 	}
	if(GetPVarInt(playerid, "timduong") == 1)
	{
		SetPVarInt(playerid, "timduong", 0);
		DisablePlayerCheckpoint(playerid);
		Player_Notice(playerid, AUTO_MESSAGE, "Ban da den dia diem");
	}
	if(GetPVarInt(playerid, "FindJob") == 1)
	{
		DeletePVar(playerid, "FindJob");
		SendClientMessage(playerid, -1, "Ban da den noi.");
	...
	for(new h = 0; h < MAX_POINTS; h++)
	{
		if(Points[h][Type] == 3 && GetPVarInt(playerid, "CrateDeliver") == 1 && IsPlayerInRangeOfPoint(playerid, 6.0, 2166.3772,-1675.3829,15.0859))
		{
			...
			else
			{
  				...
  				new randxp = 10 + random(50);
  				PlayerInfo[playerid][pXP] += randxp;
			}
	...
	if(GetPVarInt(playerid, "TruckDeliver") > 0 && gPlayerCheckpointStatus[playerid] != CHECKPOINT_RETURNTRUCK)
	{
		...
		{
			SetPlayerCheckpoint(playerid, 2413.6931,-2136.4214,13.3016, 5);
			GameTextForPlayer(playerid, "~w~Dia diem ~r~ben cang Ocean Docks", 5000, 1);
			SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Quay tro lai ben cang Ocean Docks (diem checkpoint tren radar).");
		}
		else
		{
	...
	if(gPlayerCheckpointStatus[playerid] == CHECKPOINT_DELIVERY)
	{
		...
		new string[128];
		format(string, sizeof(string), "* Ban da gui %s den %s. Tra lai xe tai toi ben cang Ocean Docks de nhan tien.", GetInventoryType(business), Businesses[business][bName]);
		...
		SetPlayerCheckpoint(playerid, 2413.6931,-2136.4214,13.3016, 5);
		GameTextForPlayer(playerid, "~w~Dia diem ~r~ben cang Ocean Docks", 5000, 1);
		SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Quay lai ben cang Ocean Docks (diem checkpoint tren radar).");
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, 2431.5337,-2112.5366,13.5469) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
			...
			{
			    if(!IsPlayerInRangeOfPoint(playerid, 6, 2413.6931,-2136.4214,13.3016) && !IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
				...
				if(level >= 0 && level <= 50) payment = 20000;
				else if(level >= 51 && level <= 100) payment =22000;
				else if(level >= 101 && level <= 200) payment = 25000;
				else if(level >= 201 && level <= 400) payment = 26000;
				else if(level >= 401) payment = 29000;
				...
				{
				    distancepay = floatmul(GetDistance(2431.5337,-2112.5366,13.5469, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				...
				if(TruckDeliveringTo[vehicleid] != INVALID_BUSINESS_ID) {
					...
					if (payment > 12000) payment = 12500;
					...
					if(truckdeliver == 5) // Weapons
					{
						if(PlayerInfo[playerid][pConnectHours] >= 2 && PlayerInfo[playerid][pWRestricted] <= 0)
						{
							...
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Spidtol cho viec van chuyen vu khi trai phep.");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	{
		new Float:X[3];
		GetActorPos(Actorvukhi, Float:X[0], Float:X[1], Float:X[2]);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		else if(DynVeh[vehicleid] != -1)
		{
			...
			GetPlayerPos(playerid, slx, sly, slz);
			if(DynVehicleInfo[DynVeh[vehicleid]][gv_igID] != INVALID_GROUP_ID && (PlayerInfo[playerid][pMember] != DynVehicleInfo[DynVeh[vehicleid]][gv_igID]) && PlayerInfo[playerid][pAdmin] == 0)
			{
				RemovePlayerFromVehicle(playerid);
				SetPlayerPos(playerid, slx, sly, slz+1.3);
				defer NOPCheck(playerid);
			...
			else if(DynVehicleInfo[DynVeh[vehicleid]][gv_igDivID] != 0 && PlayerInfo[playerid][pDivision] != DynVehicleInfo[DynVeh[vehicleid]][gv_igDivID] && PlayerInfo[playerid][pAdmin] == 0)
			{
				RemovePlayerFromVehicle(playerid);
				SetPlayerPos(playerid, slx, sly, slz+1.3);
				defer NOPCheck(playerid);
			...
			else if(DynVehicleInfo[DynVeh[vehicleid]][gv_ifID] != 0 && (PlayerInfo[playerid][pFMember] != DynVehicleInfo[DynVeh[vehicleid]][gv_ifID]) && PlayerInfo[playerid][pAdmin] == 0)
			{
				RemovePlayerFromVehicle(playerid);
				SetPlayerPos(playerid, slx, sly, slz+1.3);
				defer NOPCheck(playerid);
			}
			else if(DynVehicleInfo[DynVeh[vehicleid]][gv_irID] != 0 && (PlayerInfo[playerid][pRank] < DynVehicleInfo[DynVeh[vehicleid]][gv_irID]) && PlayerInfo[playerid][pAdmin] == 0)
			{
				RemovePlayerFromVehicle(playerid);
				SetPlayerPos(playerid, slx, sly, slz+1.3);
				defer NOPCheck(playerid);
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	else
	{
		...
		ConnectingView(playerid);
		PlayerTextDrawShow(playerid, logoLSRP[playerid][0]);
		PlayerTextDrawShow(playerid, logoLSRP[playerid][1]);
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    ...
    if(!success)
    {
    	format(string,sizeof(string),"Lenh ~r~%s~w~ khong ton tai.", cmdtext);
...
public OnPlayerCommandReceived(playerid, cmdtext[]) {
	...
	PlayerTextCMD(playerid, cmdtext);
	if(gPlayerLogged{playerid} != 1) {
